import { DashboardIcon, LogoutIcon, NotificationIcon, ProductIcon, SettingsIcon, UserIcon, WebsiteEditorIcon } from "../../../general/icons"

export const AdminNavItem = ({icon, navItemName, onClick, count}) => {
  return (
    <button 
      onClick={onClick} 
      className="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700 w-full inline-block"
    >
      {icon && icon}
      {navItemName && navItemName.length && (
        <span className="ml-3 text-sm">{navItemName}</span>        
      )}
      {count && (
...
export default function AdminSideNav() {
  return (
    <aside className="border-r" aria-label="Sidebar">
      <div className="overflow-y-auto py-4 px-3 rounded dark:bg-gray-800">
        ...
        <AdminNavItemGroup 
          group={[
            ...
            {icon: <SettingsIcon />,navItemName:'Settings'},
            {icon: <LogoutIcon  />,navItemName:'Sign Out'},
          ]}
        />
  
        <div id="dropdown-cta" className="p-4 mt-6 bg-blue-200 rounded-lg dark:bg-blue-900" role="alert">
